	...
	refresh(frm) {
		if (frm.doc.docstatus === 1) {
			frm.add_custom_button(
				...
				function () {
					frappe.model.open_mapped_doc({
						method: "export_lc.export_lc.doctype.export_lc.export_lc.make_sales_invoice",
						frm: frm,
					});
				},
				__("Create")
			...
			frm.add_custom_button(
				...
				function () {
					frappe.model.open_mapped_doc({
						method: "export_lc.export_lc.doctype.export_lc.export_lc.make_delivery_note",
						frm: frm,
					});
				},
				__("Create")
...
frappe.ui.form.on("Export LC Item", {
	item_code: function (frm, cdt, cdn) {
		...
		if (row.item_code) {
			frappe.db.get_value(
				"Item",
				row.item_code,
				["item_name", "description", "stock_uom", "brand"],
				function (r) {
					if (r) {
					...
					frappe.db.get_value("Item Price", filters, "price_list_rate", function (pr) {
						if (pr && pr.price_list_rate) {
							frappe.model.set_value(cdt, cdn, "rate", pr.price_list_rate);
						} else if (filters.currency) {
							frappe.db.get_value(
								"Item Price",
								{ item_code: row.item_code, price_list: "Standard Selling" },
								"price_list_rate",
								function (pr_fallback) {
									if (pr_fallback && pr_fallback.price_list_rate) {
										frappe.model.set_value(
											cdt,
											cdn,
											"rate",
											pr_fallback.price_list_rate
										);
									}
								}
							);
