frappe.ui.form.on("Export Proforma Invoice", {
	refresh: function (frm) {
		...
		if (frm.doc.company_contact && !frm.doc.company_phone_no) {
			frm.trigger("company_contact");
		}
		...
		if (frm.doc.company) {
			...
			frm.set_query("company_contact", function () {
				return {
		...
		// Buyer (Customer): auto-populate address/contact display and apply query filters
	...
	company: function (frm) {
		...
		frm.set_value("company_contact", "");
		frm.set_value("company_phone_no", "");
		frm.set_value("company_email", "");
		...
		// Auto-fetch contact if exactly one exists for this company
		frappe.call({
			method: "frappe.client.get_list",
			args: {
				doctype: "Contact",
			...
			callback: function (r) {
				if (r.message && r.message.length === 1) {
					frm.set_value("company_contact", r.message[0].name);
				}
			},
		});
	},
	...
	company_contact: function (frm) {
		if (frm.doc.company_contact) {
			frappe.call({
				method: "frappe.client.get",
				args: {
					doctype: "Contact",
					name: frm.doc.company_contact,
				},
				callback: function (r) {
					if (r.message) {
						...
						if (contact.phone_nos && contact.phone_nos.length) {
							let primary =
								contact.phone_nos.find((p) => p.is_primary_mobile_no) ||
						...
						if (contact.email_ids && contact.email_ids.length) {
							let primary =
								contact.email_ids.find((e) => e.is_primary) ||
								contact.email_ids[0];
							email = primary ? primary.email_id : "";
						}
						frm.set_value("company_phone_no", phone);
						frm.set_value("company_email", email);
					}
				},
			});
		} else {
	...
	buyer: function (frm) {
		...
		// Fetch phone/email directly from Customer
		frappe.call({
